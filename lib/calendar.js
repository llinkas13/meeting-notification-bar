...
/**
 * Today's date (YYYY-MM-DD) *in `tz`*, not in the process's own zone.
 *
 * Pulled out as its own function so callers can't accidentally pair it with a different zone than
 * the one they pass to tzOffset() — computing the date and the offset from two different zones is
 * exactly the silent wrong-day bug this module exists to avoid (see fetch-events.js's header).
 */
function todayIn(tz) {
...
module.exports = { listEvents, joinUrlOf, tzOffset, todayIn };
